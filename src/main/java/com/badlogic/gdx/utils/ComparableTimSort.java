...
/**
 * This is a near duplicate of {@link TimSort}, modified for use with arrays of objects that implement
 * {@link Comparable}, instead of using explicit comparators.
 *
 * <p>
 * If you are using an optimizing VM, you may find that ComparableTimSort offers no performance benefit over TimSort in
 * conjunction with a comparator that simply returns {@code ((Comparable)first).compareTo(Second)}. If this is the case,
 * you are better off deleting ComparableTimSort to eliminate the code duplication. (See Arrays.java for details.)
 */
class ComparableTimSort {
     ...
     * determined to work better in this implementation. In the unlikely event that you set this constant to be a number
     * that's not a power of two, you'll need to change the {@link #minRunLength} computation.
     *
    ...
    /**
     * This controls when we get *into* galloping mode. It is initialized to MIN_GALLOP. The mergeLo and mergeHi methods
     * nudge it higher for random data, and lower for highly structured data.
     */
     ...
     *
     * Unlike Tim's original C version, we do not allocate this much storage when sorting smaller arrays. This change
     * was required for performance.
     */
    ...
    /**
     * A stack of pending runs yet to be merged. Run i starts at address base[i] and extends for len[i] elements. It's
     * always true (so long as the indices are in bounds) that:
     *
     * runBase[i] + runLen[i] == runBase[i + 1]
     *
     * so we could cut the storage for this, but it's a minor amount, and keeping all the info explicit simplifies the
     * code.
     */
    ...
    /**
     * Asserts have been placed in if-statements for performace. To enable them, set this field to true and enable them
     * in VM with a command line flag. If you modify this class, please do test the asserts!
     */
     ...
     * @param start the index of the first element in the range that is not already known to be sorted (@code lo <=
     * start <= hi}
     */
     ...
     * will be extended with {@link #binarySort}.
     *
     * Roughly speaking, the computation is:
     *
     * If n < MIN_MERGE, return n (it's too small to bother with fancy stuff). Else if n is an exact power of 2, return
     * MIN_MERGE/2. Else return an int k, MIN_MERGE/2 <= k <= MIN_MERGE, such that n/k is close to, but strictly less
     * than, an exact power of 2.
     *
     ...
     * this method will run. @return the int k, 0 <= k <= n such that a[b + k - 1] < key <= a[b + k], pretend ing that
     * a[b - 1] is minus infinity and a[b + n] is infinity. In other words, key belongs at index b + k; or in other
     ...
     * @param len the length of the range; must be > 0
     * @param hint the index at which to begin the search, 0 <= hint < n. The closer hint is to the result, the faster
     * this method will run. @return the int k, 0 <= k <= n such that a[b + k - 1] <= key < a[b + k]
     */
     ...
     * the first element of the second run (a[base1] > a[base2]), and the last element of the first run (a[base1 +
     * len1-1]) must be greater than all elements of the second run.
     *
     * For performance, this method should be called only when len1 <= len2; its twin, mergeHi should be called if len1
     * >= len2. (Either method may be called if len1 == len2.)
     *
    ...
    /**
     * Like mergeLo, except that this method should be called only if len1 >= len2; mergeLo should be called if len1 <=
     * len2. (Either method may be called if len1 == len2.)
     *
     * @param base1 index of first element in first run to be merged
     * @param len1 length of first run to be merged (must be > 0)
